...
def load_yaml_from_git(ref: str, repo_path: Path, file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"ERROR: failed to read/parse YAML from {ref}:{file_path}: {e}",
            file=sys.stderr,
        )
        sys.exit(4)
...
def validate_schema(data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if "meta" not in data or "slots" not in data:
        errs.append("Missing 'meta' or 'slots' keys")
        return False, errs
    slots = data["slots"]
    ...
    for k, v in slots.items():
        ...
        for req in ("name", "score", "level"):
            if req not in v:
                errs.append(f"{k}: missing required field '{req}'")
        s = v.get("score")
...
def text_dashboard(
    ...
    for i in range(1, 11):
        ...
        if rationale:
            lines.append(f"- Note: {rationale[0]}")
        lines.append("")
    lines += [
        "## Averages",
