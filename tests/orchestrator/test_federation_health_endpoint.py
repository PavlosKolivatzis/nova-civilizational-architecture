...
def test_federation_health_payload():
    ...
    assert payload["peers"] == 2
    assert payload["checkpoint_height"] == 55
    assert payload["peer_details"][0]["peer"] == "node-a"
    assert payload["peer_details"][0]["up"] is True
...
def test_federation_health_handles_exception(monkeypatch):
    ...
    monkeypatch.setattr(federation_health, "collect_health", boom)
    resp = _client().get("/federation/health")
