...
def _tri_eval(strict: bool = False, **kw):
    from nova.orchestrator.adapters.slot4_tri import evaluate as tri_evaluate
    return tri_evaluate(strict=strict, **kw)

def _constellate(**kw):
    from nova.orchestrator.adapters.slot5_constellation import constellate
    return constellate(**kw)

def _synthesize_culture(mode: str = "normal", **kw):
    from nova.orchestrator.adapters.slot6_cultural import synthesize
...
def _deploy_canary(**kw):
    from nova.orchestrator.adapters.slot10_civilizational import run_canary_stage
