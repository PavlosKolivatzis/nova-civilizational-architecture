...
class ARCCalibrationRunner:
    ...
    def __init__(self, cycle_number: int, domains_file: str, output_file: str):
    ...
    def run_detection_cycle(self, domains: List[Dict[str, Any]]) -> List[DetectionResult]:
        ...
        for domain in domains:
            try:
                ...
                # Check for extraction system indicators
...
def main():
    ...
    # Default output path
    if not args.output:
        args.output = f'data/arc_cycle_{args.cycle}_results.json'

    # Run calibration cycle
    runner = ARCCalibrationRunner(args.cycle, args.domains, args.output)
    runner.run()
